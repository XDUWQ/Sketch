...
class MonNet(object):
	...
	def build_network(self, names, sources, targets, masks, angles, views, is_training=False, is_validation=False, is_testing=False, is_encoding=False):
		...
		if not self.config.continuous_view:
			with tf.compat.v1.variable_scope(var_scope_G):
		...
		if not self.config.no_adversarial:
			with tf.compat.v1.variable_scope(var_scope_D):
		...
		if self.config.no_adversarial:
			...
			loss_g_a = tf.reduce_sum(-tf.math.log(tf.maximum(probs_preds, 1e-6)))
			loss_d_r = tf.reduce_sum(-tf.math.log(tf.maximum(probs_targets, 1e-6)))
			loss_d_f = tf.reduce_sum(-tf.math.log(tf.maximum(1.0-probs_preds, 1e-6)))
		...
		if is_validation:
			...
			self.valid_summary_losses = tf.compat.v1.placeholder(tf.float32, shape=self.valid_losses.get_shape())
			vG_all, vG_p, vG_a, vD_all, vD_r, vD_f = tf.unstack(self.valid_summary_losses)
			tf.compat.v1.summary.scalar('vG_all', vG_all, collections=[valid_summary_name])
			tf.compat.v1.summary.scalar('vG_p',   vG_p,   collections=[valid_summary_name])
			tf.compat.v1.summary.scalar('vG_a',   vG_a,   collections=[valid_summary_name])
			tf.compat.v1.summary.scalar('vD_all', vD_all, collections=[valid_summary_name])
			tf.compat.v1.summary.scalar('vD_r',   vD_r,   collections=[valid_summary_name])
			tf.compat.v1.summary.scalar('vD_f',   vD_f,   collections=[valid_summary_name])
			self.valid_summary_op = tf.compat.v1.summary.merge_all(valid_summary_name)
		...
		tf.compat.v1.summary.scalar('G_all', loss_G,   collections=[train_summary_G_name])
		tf.compat.v1.summary.scalar('G_p',   loss_g_p, collections=[train_summary_G_name])
		tf.compat.v1.summary.scalar('G_a',   loss_g_a, collections=[train_summary_G_name])
		tf.compat.v1.summary.scalar('D_all', loss_D,   collections=[train_summary_D_name])
		tf.compat.v1.summary.scalar('D_r',   loss_d_r, collections=[train_summary_D_name])
		tf.compat.v1.summary.scalar('D_f',   loss_d_f, collections=[train_summary_D_name])
		...
		learning_rate = tf.compat.v1.train.exponential_decay(init_learning_rate, global_step=opt_step, decay_steps=10000, decay_rate=0.96, staircase=True)
		
		opt_G = tf.compat.v1.train.AdamOptimizer(learning_rate=learning_rate, beta1=adam_beta1, beta2=adam_beta2, name='ADAM_G')
		opt_D = tf.compat.v1.train.AdamOptimizer(learning_rate=learning_rate, beta1=adam_beta1, beta2=adam_beta2, name='ADAM_D')
		...
		self.grad_G_placeholder = [(tf.compat.v1.placeholder(tf.float32, shape=grad[1].get_shape()), grad[1]) for grad in grad_G if grad[0] is not None]
		...
		if not self.config.no_adversarial:
			...
			self.grad_D_placeholder = [(tf.compat.v1.placeholder(tf.float32, shape=grad[1].get_shape()), grad[1]) for grad in grad_D if grad[0] is not None]
		...
		tf.compat.v1.summary.image('result', result_tile, 12, [train_summary_G_name])

		self.train_summary_G_op = tf.compat.v1.summary.merge_all(train_summary_G_name)
		self.train_summary_D_op = tf.compat.v1.summary.merge_all(train_summary_D_name)
		...
		bn_G_collection = tf.compat.v1.get_collection(bn_scope_G)
		bn_D_collection = tf.compat.v1.get_collection(bn_scope_D)
	...
	def train(self, sess, views, num_train_shapes, num_valid_shapes):
		...
		if ckpt and ckpt.model_checkpoint_path:
			self.saver = tf.compat.v1.train.Saver(keep_checkpoint_every_n_hours=10.0, max_to_keep=2)
		...
		self.summarizer = tf.compat.v1.summary.FileWriter(self.config.train_dir, sess.graph)
	...
	def test(self, sess, views, num_shapes):
		...
		coord = tf.train.Coordinator()
		threads = tf.train.start_queue_runners(sess=sess, coord=coord)
		self.summarizer = tf.compat.v1.summary.FileWriter(self.config.test_dir, sess.graph)
