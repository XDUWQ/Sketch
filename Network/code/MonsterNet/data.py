...
def load_data(config, views, shape_list, shuffle=True, batch_size=-1):
	...
	source_images = [tf.image.decode_png(tf.io.read_file(file), channels=1, dtype=tf.uint8) for file in source_files]
	...
	if not config.test:
		target_images = tf.stack([tf.image.decode_png(tf.io.read_file(file), channels=4, dtype=tf.uint16) for file in target_files])
